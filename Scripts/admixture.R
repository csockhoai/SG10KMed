...
# Generated outcomes are displayed in Figures 2A-B, Extended Data Figures 3-4, Supplementary Tables 7-9
...
# To generate barplot for comparing maxQ levels of R/E mismatched and R/E matched individuals (Extended Data Figure 3)
...
# calculate the median maxQ of R/E mismatch and R/E match group, and statistically test the difference
maxq %>% group_by(RE_Status) %>% summarise(Median = median(maxQ))
wilcox.test(maxq$maxQ~maxq$RE_Status)$p.value

# count R/E mismatch & match individuals and summarise descriptive statistics (Supplementary Table 7)
...
# To generate list of ancestry-specific variants
...
all_tier1_pathogenic_variants_summary <- read.table("Variant_Level_r5.3_20211117.txt", header = TRUE, sep = "\t")
variant_summary_subset_r5p3_9051 <- all_tier1_pathogenic_variants_summary %>% filter(Total >= 5)

m_spec_var_r5p3 <- c_spec_var_r5p3 <- i_spec_var_r5p3 <- c()
for(i in 1:nrow(variant_summary_subset_r5p3_9051)){
  myrow <- variant_summary_subset_r5p3_9051[i,]
  #We consider a variant to be re specific if is 1) the most out of the 3 REs and 2) 10x higher CF compared to both other
  if(which.max(c(myrow$C, myrow$I, myrow$M)) == 3){
    if(myrow$C > 0){
      mc_ratio <- myrow$M_Frac / myrow$C_Frac  
    }else{
      mc_ratio <- 100
    }
    if(myrow$I > 0){
      mi_ratio <- myrow$M_Frac / myrow$I_Frac  
    }else{
      mi_ratio <- 100
    }
    if(mc_ratio >= 10 & mi_ratio >= 10 & (myrow$C > 0 | myrow$I > 0)){
      m_spec_var_r5p3 <- rbind(m_spec_var_r5p3,myrow)    
    }
  }
  
  if(which.max(c(myrow$C, myrow$I, myrow$M)) == 1){
    if(myrow$M > 0){
      cm_ratio <- myrow$C_Frac / myrow$M_Frac  
    }else{
      cm_ratio <- 100
    }
    if(myrow$I > 0){
      ci_ratio <- myrow$C_Frac / myrow$I_Frac  
    }else{
      ci_ratio <- 100
    }
    if(cm_ratio >= 10 & ci_ratio >= 10 & (myrow$M > 0 | myrow$I > 0)){
      c_spec_var_r5p3 <- rbind(c_spec_var_r5p3,myrow)    
    }
  }
  
  if(which.max(c(myrow$C, myrow$I, myrow$M)) == 2){
    if(myrow$M > 0){
      im_ratio <- myrow$I_Frac / myrow$M_Frac  
    }else{
      im_ratio <- 100
    }
    if(myrow$C > 0){
      ic_ratio <- myrow$I_Frac / myrow$C_Frac  
    }else{
      ic_ratio <- 100
    }
    if(ic_ratio >= 10 & im_ratio >= 10 & (myrow$M > 0 | myrow$C > 0)){
      i_spec_var_r5p3 <- rbind(i_spec_var_r5p3,myrow)    
    }
  }
}
combined_spec_var_r5p3 <- rbind(c_spec_var_r5p3, m_spec_var_r5p3, i_spec_var_r5p3)
...
# To compare maxQ in concordant vs discordant carriers (analysis of all ancestries combined) (Supplementary Table 9)
...
#Calculate odds ratio of admixed vs non-admixed individuals carrying discordant P/LP variant and the adjusted (Benjamini-Hochberg) p-values (Supplementary Table 9)
...
# To tabulate median values of ancestral components calculated above (Supplementary Table 8)
...
# To statistically test whether median maxQ of discordant variant carriers from R/E mismatch and R/E match groups are equal (Extended Figure 4)
admix <- spec_plot_df
maxq_re_v2 <- maxq %>% select(NPM_Research_ID, Supplied_Ethnicity, RE_Status, Malay_Comp, Indian_Comp, Chinese_Comp, maxQ)
admix_re_v2 <- left_join(admix, maxq_re_v2)

soi_c <- admix_re_v2 %>% filter(Specific_Ancestry == "Chinese_Specific" & Genetic_Ancestry != "Chinese" & Carrier_Status == TRUE)
soi_i <- admix_re_v2 %>% filter(Specific_Ancestry == "Indian_Specific" & Genetic_Ancestry != "Indian" & Carrier_Status == TRUE) 
soi_m <- admix_re_v2 %>% filter(Specific_Ancestry == "Malay_Specific" & Genetic_Ancestry != "Malay" & Carrier_Status == TRUE)
soi_combined <- rbind.data.frame(soi_c, soi_i, soi_m)
soi_combined2 <- unique(soi_combined %>% select(NPM_Research_ID, RE_Status, maxQ))
t.test(soi_combined2$maxQ~soi_combined2$RE_Status)
wilcox.test(soi_combined2$maxQ~soi_combined2$RE_Status)
table(soi_combined2$RE_Status)

#Calculate median maxQ values of discordant PLP variant carriers by R/E matched/mismatched status
soi_combined2 %>% group_by(RE_Status) %>% summarise(median_maxQ = median(maxQ))

# To generate barplot comparing maxQ distribution of discordant PLP variant carriers by R/E matched/mismatched status (Extended Data Figure 4)
F16 <- ggplot(soi_combined2, aes(x = RE_Status, y = maxQ, fill = RE_Status)) + geom_boxplot() + theme(
...
# To compute adjusted p-values for spec_plot_df (Supplementary Table 8)
...
i <- ggplot(i_spec, aes(x = Genetic_Ancestry, y = Ancestral_Fraction, fill = Carrier_Status)) + geom_boxplot() + theme(panel.background = element_blank(), panel.border = element_rect(fill = NA, linetype = "solid", size = 0.5), legend.position = "none")
i
